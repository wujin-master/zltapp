...
public class ExamStudentController {
    ...
    public Result showExam(@PathVariable("id") String id){
        ...
        List<String> eduUserExamIdList = eduUserExamList.stream().map(e->e.getExamId())
                                            .collect(Collectors.toList());
        List<EduExam> eduExamList = new ArrayList<>();
        EduExam eduExam = null;
        for(String examId : eduUserExamIdList){
            eduExam = examService.findExamByExamId(examId);
            eduExamList.add(eduExam);
        }
        int count = eduUserExamList.size();
        Map<String,Object> map = new HashMap<>();
        map.put("eduUserExamList",eduUserExamList);
        map.put("count",count);
    ...
    public Result getPaperContext(@PathVariable("examId") String examId){
        ...
        System.out.println("eduPaperId" + eduPaper.getPaperId());
    ...
    @ResponseBody
    public Result getScore(@RequestBody EduUserExam eduUserExam){
        EduUserExam eduUserExam1 = eduUserExamService.findByUIEI(eduUserExam.getUserId(),eduUserExam.getExamId());
        eduUserExam.setId(eduUserExam1.getId());
        int result = eduUserExamService.updUserExam(eduUserExam);
        if(result == 1){
